...
for USER_DATA in $USERS; do
    ...
    srun -p $PARTITION_A -N $NODE_COUNT_A --ntasks-per-node=1 sudo groupadd -g $GID_VAL $USERNAME 
    srun -p $PARTITION_A -N $NODE_COUNT_A --ntasks-per-node=1 sudo useradd -u $UID_VAL -g $GID_VAL -m -s /bin/bash $USERNAME 
    srun -p $PARTITION_B -N $NODE_COUNT_B --ntasks-per-node=1 sudo groupadd -g $GID_VAL $USERNAME 
    srun -p $PARTITION_B -N $NODE_COUNT_B --ntasks-per-node=1 sudo useradd -u $UID_VAL -g $GID_VAL -m -s /bin/bash $USERNAME 
done
