...
for USER_DATA in $USERS; do
    ...
    srun -partition=$PARTITION -N $NODE_COUNT --ntasks-per-node=1 sudo groupadd -g $GID_VAL $USERNAME 
    srun -partition=$PARTITION -N $NODE_COUNT --ntasks-per-node=1 sudo useradd -u $UID_VAL -g $GID_VAL -m -s /bin/bash $USERNAME 
done
