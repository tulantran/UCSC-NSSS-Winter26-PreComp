...
## `mpirun` location
You might need to specify where mpirun is located on your machine in the template instead of just typing `mpirun`. It should be in the bin folder of wherever you compiled OpenMPI, the same exact folder as `mpicc`. Write out the absolute path and that should be enough.
